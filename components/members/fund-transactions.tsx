...
export default function FundTransactions({ transactions }: FundTransactionsProps) {
  ...
  const filtered = useMemo(() => {
    return transactions.filter((t) => {
      const fund = (t.fundName || '').toLowerCase()
      if (search && !fund.includes(search.toLowerCase())) return false
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-navy-300">{filtered.length} of {transactions.length} transactions</span>
      </div>

      {/* Empty state */}
      {filtered.length === 0 ? (
        <Card bordered>
          <CardContent className="py-8 text-center text-navy-300">No fund transactions match your filters.</CardContent>
        </Card>
      ) : (
        <div className="space-y-2">
          {years.map((year) => {
            ...
            const yearInvested = yearTxs
              .filter((t) => {
                const type = (t.type || '').toLowerCase()
                return type === 'fund contribution' || type === 'fund commitment'
              })
              ...
              <div key={year}>
                ...
                >
                  ...
                  <span className="text-xs text-navy-300">
                    ...
                    {yearInvested > 0 ? ` · ${fmt(yearInvested)} contributed` : ''}
                  </span>
                </button>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-navy-100 bg-navy-50">
                            {['Date', 'Fund', 'Amount', 'Type'].map((h) => (
                              <th key={h} className="text-left text-xs font-semibold text-navy-400 uppercase tracking-wider px-4 py-3 whitespace-nowrap">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
